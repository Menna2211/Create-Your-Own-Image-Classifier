...
# Imports for PyTorch and related libraries
import torch
from torch import nn, optim
import torch.nn.functional as F

# Imports for handling image data with torchvision
import torchvision
from torchvision import datasets, transforms, models

# Imports for image processing with PIL and numerical operations with numpy
from PIL import Image
import numpy as np

# Imports for visualizing plots with matplotlib
get_ipython().run_line_magic('matplotlib', 'inline')
get_ipython().run_line_magic('config', "InlineBackend.figure_format = 'retina'")
import matplotlib.pyplot as plt

# Imports for working with JSON files and ordered dictionaries
import json
...
validation_test_trans = transforms.Compose([transforms.Resize(255),
...
print('The number of classes is ',len(cat_to_name))
...
# Assuming train_losses and valid_losses are lists, convert them to PyTorch tensors
train_losses = torch.tensor(train_losses)
valid_losses = torch.tensor(valid_losses)

# Plotting the training and validation losses
plt.plot(train_losses.cpu().numpy(), label='Training loss')  # Plotting training loss
plt.plot(valid_losses.cpu().numpy(), label='Validation loss')  # Plotting validation loss

# Adding title and labels to the plot
plt.title('Training and Validation Losses')  # Adding a title to the plot
plt.xlabel('Epochs')  # Label for the x-axis
plt.ylabel('Loss')  # Label for the y-axis

# Adding legend and displaying the plot
plt.legend(frameon=False)  # Displaying legend without a frame
...
imshow(image, ax=ax1)
ax1.axis('off')
ax1.set_title(cat_to_name[path.split('/')[-2]])

# Extract prediction probabilities and class labels
probs, classes = predict(image_path, model)

# Map class indices to class names
class_names = [cat_to_name[str(cls)] for cls in classes]

# Bar graph for the top 5 classes

ax2.barh(class_names, probs[0])
